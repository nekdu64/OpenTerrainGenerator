...
public class SpigotMaterialReader implements IMaterialReader
{
	...
	//this one is used for BO3 and BO4 if a material is not recognise it will be replaced by defaultBlock (IfBlockError:) in specific BO3config files
	@Override
	public LocalMaterialData readMaterial(String material, LocalMaterialData defaultBlock, String BO3name) throws InvalidConfigException {
		{
			...
			try
			{
				localMaterial = materialFromString(material,defaultBlock,BO3name);
			}
			catch (InvalidConfigException ex)
			{
				// Happens when a non existing block name is used.
	...
	private LocalMaterialData materialFromString(String input,LocalMaterialData defaultBlock, String BO3name) throws InvalidConfigException
	{
		...
		if(defaultBlock==null){
			defaultBlock = SpigotMaterialData.ofBlockData(Blocks.NOTE_BLOCK.getBlockData(), input);
		}

		if(OTG.getEngine().getLogger().getLogCategoryEnabled(LogCategory.CONFIGS))
		{
			OTG.getEngine().getLogger().log(LogLevel.ERROR, LogCategory.CONFIGS, "Could not parse block: " + input + " (" + blockNameCorrected + "),in "+ BO3name + " in substituting " + defaultBlock.getName());
		}
