...
public abstract class BlockFunction<T extends CustomObjectConfigFile> extends CustomObjectConfigFunction<T>
{
	...
	public void load(List<String> args, ILogger logger, IMaterialReader materialReader) throws InvalidConfigException
	{
		...
		//check if config is BO3/BO4 to use the correct readMaterial methode
		if (getHolder() instanceof BO3Config) {
			BO3Config holder = (BO3Config) getHolder();
			material = readMaterial(holder.getName(),holder.ifBlockError, args.get(3), materialReader);
		}
		if (getHolder() instanceof BO4Config) {
			BO4Config holder = (BO4Config) getHolder();
			material = readMaterial(holder.getName(),holder.ifBlockError, args.get(3), materialReader);
		}
		else {
			material = readMaterial(holder.getName(),null,args.get(3), materialReader);
		}
		if(material == null)
		{
